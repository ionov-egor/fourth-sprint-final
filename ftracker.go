...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	speed := meanSpeed(action, duration) // Скоросто в км/ч
	return ((18 * speed * 1.79) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speed, 2.0)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
